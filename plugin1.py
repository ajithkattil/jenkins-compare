...
def match_shared_libraries(master,remote,remote_host,remote_user,remote_password):  
   ...
   ssh = pxssh.pxssh()
   ssh.login(remote_host,remote_user,remote_password)
...
def check_maven_installations(master,remote,remote_host,remote_user,remote_password):
   ...
   for i in m1.find('installations').iter('hudson.tasks.Maven_-MavenInstallation'):
      count = 0    
      for j in m2.find('installations').iter('hudson.tasks.Maven_-MavenInstallation'):
         if i.find('home').text == j.find('home').text:
            print("Maven version matched  -->"+i.find('home').text)
            f2.write("Maven version matched ---> "+","+i.find('home').text+'\n')
            count = 1
      if count == 0:
         print("Maven version Unavailable --> "+i.find('home').text)
         f2.write("Maven version Unavailable ---> "+","+i.find('home').text+'\n')
